...
function sanitizeInfo(info) {
    ...
    return {
        nickname: sanitizeText(info.nickname || 'PLAYER', 8),
...
const server = net.createServer((socket) => {
    ...
    function handleMessage(socket, msg) {
        ...
        else if (msg.type === 'game_sync' || msg.type === 'turn' || msg.type === 'shoot' || msg.type === 'pull' || msg.type === 'pull_end' || msg.type === 'end_turn' || msg.type === 'typing') {
            // Pure relay: the server never inspects or simulates gameplay, it just
            // forwards small packets to the other player. 'game_sync' carries
            // everything gameplay-related now — a whole settled turn
            // (kind="turn": shot + rest snapshot), the coin toss and end-of-match
            // decisions. No per-frame streaming, so CPU stays at a few % even with
